...
import {
  ...
  searchProducts, addStock, getStockHistory, editStockEntry, deleteStockEntry,
  type CreateProductDTO, type UpdateProductDTO, type AddStockDTO,
  type EditStockEntryDTO, type Product, type StockEntry, type Quantity,
} from "../api/product";
...
const emptyProductForm = {
  ...
  quantityValue: "", quantityUnit: "piece" as string,
  description: "", flavour: "",
};

const emptyStockForm = { count: "", price: "", retailPrice: "", note: "", stockDate: "" };
...
export default function Products() {
  ...
  const [modal, setModal] = useState<
    "create"|"edit"|"delete"|"view"|"stock"|"stockHistory"|"editEntry"|"deleteEntry"|null
  >(null);
  ...
  const [entryForm, setEntryForm] = useState({
    count: "", remainingCount: "", price: "", retailPrice: "", note: "", stockDate: "",
  });
  ...
  const load = useCallback(async () => {
    ...
    try {
      if (searchDebounced.trim()) {
        ...
        setProducts(arr); setTotal(arr.length); setPages(1);
      } else {
  ...
  /* ── modal openers ── */
  const openCreate = () => {
    setProductForm({ ...emptyProductForm }); setFormError(""); setModal("create");
  };
  ...
  const openAddStock     = (p: Product)      => {
    setSelected(p); setStockForm({ ...emptyStockForm }); setFormError(""); setModal("stock");
  };
  const openStockHistory = async (p: Product) => {
    setSelected(p); setModal("stockHistory"); setStockLoading(true);
    try   { const res = await getStockHistory(p._id); setStockEntries(res.entries); }
  ...
  const openEditEntry   = (entry: StockEntry) => {
    ...
    setFormError(""); setModal("editEntry");
  };
  const openDeleteEntry = (entry: StockEntry) => {
    setSelectedEntry(entry); setModal("deleteEntry");
  };
  const closeModal = () => {
    setModal(null); setSelected(null); setSelectedEntry(null); setStockEntries([]);
  };

  /* ── create product ── */
  const handleCreate = async () => {
    if (!productForm.name || !productForm.brandId || !productForm.categoryId) {
      setFormError("Name, brand and category are required."); return;
    }
    setSubmitting(true);
    try {
      const dto: CreateProductDTO = {
        name: productForm.name, brandId: productForm.brandId, categoryId: productForm.categoryId,
    ...
    } catch (e: any) { setFormError(e?.response?.data?.message || e.message || "Failed to create product"); }
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      showToast("Product updated", "success"); closeModal(); load();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } catch (e: any) { setFormError(e?.response?.data?.message || e.message || "Failed to update product"); }
    finally { setSubmitting(false); }
  };
  ...
  const handleDelete = async () => {
    if (!selected) return;
    setSubmitting(true);
    try   { await deleteProduct(selected._id); showToast("Product deleted", "success"); closeModal(); load(); }
  ...
  const handleAddStock = async () => {
    if (!selected) return;
    if (!stockForm.count || Number(stockForm.count) < 1) { setFormError("Count must be at least 1."); return; }
    if (isAdmin && !stockForm.price) { setFormError("Admin must set a cost price for this batch."); return; }
    setSubmitting(true);
    try {
      ...
      showToast("Stock batch added successfully", "success"); closeModal(); load();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } catch (e: any) { setFormError(e?.response?.data?.message || e.message || "Failed to add stock"); }
    finally { setSubmitting(false); }
  };
  ...
  const handleEditEntry = async () => {
    ...
    if (entryForm.count === ""          || countVal     < 0) { setFormError("Count cannot be negative.");                    return; }
    if (entryForm.remainingCount === "" || remainingVal < 0) { setFormError("Remaining count cannot be negative.");          return; }
    if (remainingVal > countVal)                             { setFormError("Remaining count cannot exceed total count.");   return; }
    setSubmitting(true);
    try {
      const dto: EditStockEntryDTO = {
        count: countVal, remainingCount: remainingVal,
      ...
      setStockEntries(res.entries); load(); setSelectedEntry(null); setFormError(""); setModal("stockHistory");
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } catch (e: any) { setFormError(e?.response?.data?.message || e.message || "Failed to update entry"); }
    finally { setSubmitting(false); }
  };
  ...
  const handleDeleteEntry = async () => {
    ...
    try {
      ...
      const res = await getStockHistory(selected._id);
      setStockEntries(res.entries); load(); setSelectedEntry(null); setModal("stockHistory");
    } catch { showToast("Failed to delete stock entry", "error"); }
    finally { setSubmitting(false); }
  };

  /* ── field helpers ── */
  const pf  = (k: keyof typeof emptyProductForm) => productForm[k];
  const setPF = (k: keyof typeof emptyProductForm, v: string) => setProductForm(p => ({ ...p, [k]: v }));
  const sf  = (k: keyof typeof emptyStockForm)   => stockForm[k];
  const setSF = (k: keyof typeof emptyStockForm, v: string)   => setStockForm(p => ({ ...p, [k]: v }));
  ...
  const stockFormFields = (
    <div className="form-grid">
      <div className="form-group full">
        ...
        <input className="finput" type="date" value={sf("stockDate")} onChange={e => setSF("stockDate", e.target.value)} />
      </div>
      <div className="form-group full">
  ...
  return (
    <>
      <div className="pr-root">
        ...
        <main className="main">
          <PageHeader
            ...
            subtitle={loading ? "Loading…" : `${total} product${total !== 1 ? "s" : ""} found`}
          ...
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>Product</th><th>Category</th><th>Price</th><th>Quantity</th><th>Stock</th><th>Actions</th></tr>
              </thead>
              <tbody>
                {loading
                      ...
                      return (
                        <tr key={p._id}>
                          <td>
                            <div className="prod-name" onClick={() => openView(p)}>
                              <div className="product-name-wrap">
                                ...
                                {p.flavour && p.flavour !== "none" && <span className="flavour-badge">{p.flavour}</span>}
                              </div>
                            </div>
                          ...
                          <td>
                            ...
                            {isAdmin && latestPrice != null && <div className="price-cost-secondary">Cost: {fmt(latestPrice)}</div>}
                            {isAdmin && batches > 1 && p.priceSummary && (
                              <div className="price-range-hint">{fmt(p.priceSummary.priceRange.min)} – {fmt(p.priceSummary.priceRange.max)}</div>
                            )}
                          </td>
                          <td><span className="qty-val">{p.quantity ? `${p.quantity.value} ${p.quantity.unit}` : "—"}</span></td>
                          <td>
                            ...
                            {batches > 0 && <div className="batch-hint">{batches} batch{batches !== 1 ? "es" : ""}</div>}
                          </td>
                          <td>
      ...
      {/* ── CREATE ── */}
      {modal === "create" && (
        <Modal title="Add New Product" onClose={closeModal} footer={
          <>
            <button className="mbtn cancel" onClick={closeModal}>Cancel</button>
            <button className="mbtn primary" disabled={submitting} onClick={handleCreate}>{submitting ? "Creating…" : "Create Product"}</button>
          </>
        }>
          {productFormFields}
      ...
      {/* ── EDIT ── */}
      {modal === "edit" && (
        <Modal title="Edit Product" onClose={closeModal} footer={
          <>
            <button className="mbtn cancel" onClick={closeModal}>Cancel</button>
            <button className="mbtn primary" disabled={submitting} onClick={handleEdit}>{submitting ? "Saving…" : "Save Changes"}</button>
          </>
        }>
          {productFormFields}
          {formError && <p className="ferr" style={{ marginTop: 14 }}>{formError}</p>}
      ...
      {modal === "stock" && selected && (
        <Modal
          ...
          subtitle={`${selected.name}${selected.flavour && selected.flavour !== "none" ? ` · ${selected.flavour}` : ""}`}
          size="modal-sm"
          onClose={closeModal}
          footer={
            <>
              <button className="mbtn cancel" onClick={closeModal}>Cancel</button>
              <button className="mbtn primary" disabled={submitting} onClick={handleAddStock}>{submitting ? "Adding…" : "Add Stock Batch"}</button>
            </>
          }
        >
          {stockFormFields}
        ...
        <Modal
          ...
          footer={
            <>
              ...
              <button className="mbtn primary" onClick={() => { closeModal(); openAddStock(selected); }}>+ Add New Batch</button>
            </>
          }
        >
          <div className="stock-summary-row">
            <div className="stock-summary-chip">
              ...
              <span className="ssc-val" style={{ color: stockColor(selected.currentStock ?? 0).color }}>{selected.currentStock ?? 0}</span>
            </div>
            <div className="stock-summary-chip">
          ...
          {stockLoading ? (
            Array.from({ length: 3 }).map((_, i) => <div key={i} className="sk" style={{ height: 48, borderRadius: 8, marginBottom: 8 }} />)
          ) : stockEntries.length === 0 ? (
            <div className="empty" style={{ padding: "32px 0" }}>
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 16V8a2 2 0 0 0-1-1.73l-7-4a2 2 0 0 0-2 0l-7 4A2 2 0 0 0 3 8v8a2 2 0 0 0 1 1.73l7 4a2 2 0 0 0 2 0l7-4A2 2 0 0 0 21 16z"/></svg>
              <h3>No stock batches yet</h3><p>Add the first batch to track inventory</p>
            </div>
          ) : (
            <div className="stock-entries-table-wrap">
              <table className="stock-entries-table">
                <thead>
                  <tr>
                    <th>#</th><th>Stock Date</th><th>Count</th><th>Remaining</th>
                    {isAdmin && <th>Cost Price</th>}
                    <th>Retail Price</th><th>Note</th><th>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {stockEntries.map((e, idx) => (
                    <tr key={e._id}>
                      ...
                      <td style={{ fontSize: 12, color: "#94a3b8" }}>{fmtDate(e.stockDate || e.createdAt)}</td>
                      <td><span className="batch-count">{e.count}</span></td>
                      <td>
                        <span className="batch-remaining" style={{ color: e.remainingCount === 0 ? "#ef4444" : e.remainingCount < e.count * 0.2 ? "#f59e0b" : "#10b981" }}>
                          {e.remainingCount}
                        </span>
                      </td>
                      {isAdmin && <td className="price-cell">{e.price != null ? fmt(e.price) : <span className="price-unset">Not set</span>}</td>}
                      <td className="price-cell">{e.retailPrice != null ? fmt(e.retailPrice) : "—"}</td>
      ...
      {modal === "editEntry" && selectedEntry && selected && (
        <Modal
          ...
          subtitle={`#${stockEntries.length - stockEntries.findIndex(e => e._id === selectedEntry._id)} · ${selected.name}`}
          size="modal-sm"
          onClose={() => { setModal("stockHistory"); setFormError(""); }}
          footer={
            <>
              <button className="mbtn cancel" onClick={() => { setModal("stockHistory"); setFormError(""); }}>Cancel</button>
              <button className="mbtn primary" disabled={submitting} onClick={handleEditEntry}>{submitting ? "Saving…" : "Save Changes"}</button>
            </>
          }
        >
          <div className="form-grid">
            <div className="form-group full">
              <label className="flabel">Stock Date</label>
              <input className="finput" type="date" value={entryForm.stockDate} onChange={e => setEntryForm(p => ({ ...p, stockDate: e.target.value }))} />
            </div>
            <div className="form-group">
              <label className="flabel">Count *</label>
              <input className="finput" type="number" min="0" value={entryForm.count} onChange={e => setEntryForm(p => ({ ...p, count: e.target.value }))} />
            </div>
            <div className="form-group">
              <label className="flabel">Remaining Count *</label>
              <input className="finput" type="number" min="0" max={entryForm.count} placeholder={`0 – ${entryForm.count}`} value={entryForm.remainingCount} onChange={e => setEntryForm(p => ({ ...p, remainingCount: e.target.value }))} />
            </div>
            {isAdmin && (
              <div className="form-group">
                <label className="flabel">Cost Price (₹)</label>
                <input className="finput" type="number" min="0" placeholder="0.00" value={entryForm.price} onChange={e => setEntryForm(p => ({ ...p, price: e.target.value }))} />
              </div>
            )}
            <div className="form-group">
              <label className="flabel">Retail Price (₹)</label>
              <input className="finput" type="number" min="0" placeholder="0.00" value={entryForm.retailPrice} onChange={e => setEntryForm(p => ({ ...p, retailPrice: e.target.value }))} />
            </div>
            <div className="form-group full">
              <label className="flabel">Note</label>
              <input className="finput" placeholder="e.g. Supplier: XYZ, Batch #42" value={entryForm.note} onChange={e => setEntryForm(p => ({ ...p, note: e.target.value }))} />
            </div>
          </div>
      ...
      {modal === "deleteEntry" && selectedEntry && selected && (
        <Modal
          ...
          size="modal-sm"
          onClose={() => setModal("stockHistory")}
          footer={
            <>
              <button className="mbtn cancel" onClick={() => setModal("stockHistory")}>Cancel</button>
              <button className="mbtn danger" disabled={submitting} onClick={handleDeleteEntry}>{submitting ? "Deleting…" : "Delete Batch"}</button>
            </>
          }
        >
      ...
      {modal === "view" && selected && (
        <Modal
          ...
          footer={
            <>
              <button className="mbtn cancel" onClick={closeModal}>Close</button>
              <button className="mbtn secondary" onClick={() => { closeModal(); openStockHistory(selected); }}>Stock History</button>
              <button className="mbtn primary" onClick={() => { closeModal(); openEdit(selected); }}>Edit Product</button>
            </>
          }
        >
          <div className="detail-grid">
            <div className="detail-item"><span className="detail-label">Brand</span><span className="detail-val">{selected.brand?.name ?? "—"}</span></div>
            <div className="detail-item"><span className="detail-label">Category</span><span className="detail-val">{selected.category?.name ?? "—"}</span></div>
            <div className="detail-item"><span className="detail-label">Quantity</span><span className="detail-val">{selected.quantity ? `${selected.quantity.value} ${selected.quantity.unit}` : "—"}</span></div>
            {selected.flavour && selected.flavour !== "none" && (
              <div className="detail-item"><span className="detail-label">Flavour</span><span className="detail-val">{selected.flavour}</span></div>
            )}
            ...
            <div className="detail-item">
              ...
              <span className="detail-val" style={{ color: stockColor(selected.currentStock ?? 0).color }}>
                ...
                <span style={{ fontSize: 12, opacity: .7, marginLeft: 6 }}>({stockColor(selected.currentStock ?? 0).label})</span>
              </span>
            </div>
            <div className="detail-item"><span className="detail-label">Active Batches</span><span className="detail-val">{selected.stockSummary?.totalBatches ?? 0}</span></div>
            {selected.stockSummary?.lastRestocked && (
              <div className="detail-item"><span className="detail-label">Last Restocked</span><span className="detail-val" style={{ fontSize: 13, color: "#94a3b8" }}>{fmtDate(selected.stockSummary.lastRestocked)}</span></div>
            )}
            <hr className="detail-divider" />
            {selected.priceSummary?.latestRetailPrice != null && (
              <div className="detail-item"><span className="detail-label">Retail Price</span><span className="detail-val mono">{fmt(selected.priceSummary.latestRetailPrice)}</span></div>
            )}
            {isAdmin && selected.priceSummary && (
              <>
                <div className="detail-item">
                  <span className="detail-label">Latest Cost Price<span className="admin-only-tag">Admin</span></span>
                ...
                {selected.priceSummary.priceRange.min !== selected.priceSummary.priceRange.max && (
                  <div className="detail-item full">
                    ...
                    <span className="detail-val mono">{fmt(selected.priceSummary.priceRange.min)} – {fmt(selected.priceSummary.priceRange.max)}</span>
                  </div>
                )}
              </>
            )}
            ...
            <div className="detail-item">
              ...
              <span className="detail-val" style={{ fontSize: 12, fontWeight: 400, color: "#94a3b8" }}>{selected.createdAt ? fmtDate(selected.createdAt) : "—"}</span>
            </div>
          </div>
        </Modal>
      )}
      ...
      {modal === "delete" && selected && (
        <Modal
          ...
          footer={
            <>
              <button className="mbtn cancel" onClick={closeModal}>Cancel</button>
              <button className="mbtn danger" disabled={submitting} onClick={handleDelete}>{submitting ? "Deleting…" : "Delete Product"}</button>
            </>
          }
        >
          <p style={{ fontSize: 14, color: "#94a3b8", lineHeight: 1.7 }}>
            Are you sure you want to delete{" "}
            <strong style={{ color: "#f1f5f9" }}>{selected.name}</strong>? This will mark it as deleted and cannot be undone.
          </p>
        </Modal>
      )}
