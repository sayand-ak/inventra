...
const stockColor = (n: number) => {
  if (n === 0)
    return { color: "#ef4444", bg: "rgba(239,68,68,0.1)", label: "Out" };
  if (n <= 10)
    return { color: "#f59e0b", bg: "rgba(245,158,11,0.1)", label: "Low" };
...
function toProductImages(raw: unknown): ProductImage[] {
  ...
  return raw.map((item) => {
    ...
    if (typeof item === "object" && item !== null) {
      const obj = item as { url: string; publicId?: string };
      return { url: obj.url, publicId: obj.publicId };
    }
    return { url: "" };
  });
}
...
export default function Products() {
  ...
  const openEdit = (p: Product) => {
    ...
    const existingImages: ProductImage[] = ((p as any).images ?? []).map(
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (img: any) => ({
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      if (productForm.flavour) formData.append("flavour", productForm.flavour);
      if (productForm.quantityValue) {
        formData.append(
          "quantity",
          JSON.stringify({
            value: Number(productForm.quantityValue),
            unit: productForm.quantityUnit,
          }),
        );
      }
  ...
  const handleEdit = async () => {
    ...
    try {
      ...
      // Always send all fields (simpler, backend ignores unchanged values)
      formData.append("name", productForm.name);
      formData.append("brandId", productForm.brandId);
      formData.append("categoryId", productForm.categoryId);
      formData.append("description", productForm.description || "");
      formData.append("flavour", productForm.flavour || "");
      ...
      // Existing server images — send url + publicId so backend can diff & delete removed ones
  ...
  const handleAddStock = async () => {
    ...
    try {
      const dto: AddStockDTO = {
        ...
        ...(stockForm.retailPrice
          ? { retailPrice: Number(stockForm.retailPrice) }
          : {}),
      };
      ...
      setFormError(
        e?.response?.data?.message || e.message || "Failed to add stock",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const handleEditEntry = async () => {
    ...
    try {
      const dto: EditStockEntryDTO = {
        ...
        ...(entryForm.retailPrice
          ? { retailPrice: Number(entryForm.retailPrice) }
          : {}),
      };
      ...
      setFormError(
        e?.response?.data?.message || e.message || "Failed to update entry",
      );
    } finally {
      setSubmitting(false);
    }
  ...
  const setSF = (k: keyof typeof emptyStockForm, v: string) =>
    setStockForm((p) => ({ ...p, [k]: v }));

  /* ── product form fields (shared by create & edit) ── */
  const productFormFields = (
    <div className="form-grid">
      {/* Image uploader — sits at the top, full width */}
      ...
      <div className="form-group">
        ...
        <select
          className="finput"
          value={pf("brandId")}
          onChange={(e) => setPF("brandId", e.target.value)}
        >
      ...
      <div className="form-group">
        ...
        <select
          className="finput"
          value={pf("categoryId")}
          onChange={(e) => setPF("categoryId", e.target.value)}
        >
      ...
      <div className="form-group">
        ...
        <input
          className="finput"
          placeholder="e.g. Chocolate, Orange"
          value={pf("flavour")}
          onChange={(e) => setPF("flavour", e.target.value)}
        />
      </div>
      <div className="form-group">
        ...
        <input
          ...
          min="0"
          placeholder="e.g. 2"
          value={pf("quantityValue")}
          onChange={(e) => setPF("quantityValue", e.target.value)}
        />
      </div>
      <div className="form-group">
        ...
        <select
          className="finput"
          value={pf("quantityUnit")}
          onChange={(e) => setPF("quantityUnit", e.target.value)}
        >
          {UNITS.map((u) => (
      ...
      <div className="form-group full">
        <div className="info-hint">
          ...
          Price and stock quantity are managed per stock batch. Use{" "}
          <strong>Add Stock</strong> after saving.
        </div>
      </div>
    </div>
  );
    ...
    <div className="form-grid">
      ...
      <div className="form-group full">
        ...
        <input
          ...
          placeholder="e.g. 50"
          value={sf("count")}
          onChange={(e) => setSF("count", e.target.value)}
        />
      </div>
      {isAdmin && (
        <div className="form-group">
      ...
      <div className="form-group">
        ...
        <input
          ...
          min="0"
          placeholder="0.00"
          value={sf("retailPrice")}
          onChange={(e) => setSF("retailPrice", e.target.value)}
        />
      </div>
      <div className="form-group full">
        ...
        <input
          ...
          value={sf("note")}
          onChange={(e) => setSF("note", e.target.value)}
        />
      </div>
    </div>
  );
    ...
    <>
        ...
        <main className="main">
          ...
          <div className="table-wrap">
            <table>
              <thead>
                <tr>
                  <th style={{ width: 56 }}>{/* image col */}</th>
                  <th>Product</th>
                  <th>Category</th>
                  <th>Price</th>
                  <th>Quantity</th>
              ...
              <tbody>
                {loading ? (
                  Array.from({ length: 6 }).map((_, i) => (
                    <tr key={i} className="skeleton-row">
                      ...
                      <td>
                        <div
                          className="sk"
                          style={{ height: 22, width: 60, borderRadius: 8 }}
                        />
                      </td>
                      <td>
                        <div
                          className="sk"
                          style={{ height: 30, width: 90, borderRadius: 8 }}
                        />
                      </td>
                    </tr>
                  ))
                  ...
                  products.map((p) => {
                    const stock =
                      p.currentStock ?? p.stockSummary?.currentStock ?? 0;
                    ...
                    // eslint-disable-next-line @typescript-eslint/no-explicit-any
                    const pImages = toProductImages((p as any).images);

                    return (
                      <tr key={p._id}>
                        {/* Thumbnail */}
                        <td>
                          <ProductImageStrip images={pImages} variant="thumb" />
                        </td>
                        ...
                        <td>
                          {latestRetail != null ? (
                            <div className="price-retail-primary">
                              {fmt(latestRetail)}
                            </div>
                          ) : (
                            <div className="price-retail-primary price-empty">
                              —
                            </div>
                          )}
                          {isAdmin && latestPrice != null && (
                            <div className="price-cost-secondary">
                              Cost: {fmt(latestPrice)}
                            </div>
                          )}
                          {isAdmin && batches > 1 && p.priceSummary && (
                            <div className="price-range-hint">
                              {fmt(p.priceSummary.priceRange.min)} –{" "}
                              {fmt(p.priceSummary.priceRange.max)}
                            </div>
                          )}
                        </td>
                        <td>
      ...
      {modal === "create" && (
        <Modal
          ...
          footer={
            <>
              <button className="mbtn cancel" onClick={closeModal}>
                Cancel
              </button>
              <button
        ...
        >
          {productFormFields}
          {formError && (
            <p className="ferr" style={{ marginTop: 14 }}>
              {formError}
            </p>
          )}
        </Modal>
        ...
        >
          ...
          {!isAdmin && (
            <div className="info-hint" style={{ marginTop: 12 }}>
              ...
              Cost price and retail price will be set by Admin after this batch
              is recorded.
            </div>
          )}
          {formError && (
        ...
        >
          <div className="stock-summary-row">
            <div className="stock-summary-chip">
              ...
              <span
                className="ssc-val"
                style={{ color: stockColor(selected.currentStock ?? 0).color }}
              >
            ...
            <div className="stock-summary-chip">
              ...
              <span className="ssc-val">
                {selected.stockSummary?.totalBatches ?? "—"}
              </span>
            </div>
            {selected.priceSummary?.latestRetailPrice != null && (
              <div className="stock-summary-chip">
                ...
                <span className="ssc-val">
                  {fmt(selected.priceSummary.latestRetailPrice)}
                </span>
              </div>
            )}
          </div>
                  ...
                  {stockEntries.map((e, idx) => (
                    <tr key={e._id}>
                      <td className="batch-num">
                        #{stockEntries.length - idx}
                      </td>
                      <td style={{ fontSize: 12, color: "#94a3b8" }}>
                        {fmtDate(e.stockDate || e.createdAt)}
                      </td>
                      <td>
                        <span className="batch-count">{e.count}</span>
                      </td>
                      <td>
                        <span
                      ...
                      {isAdmin && (
                        <td className="price-cell">
                          {e.price != null ? (
                            ...
                            <span className="price-unset">Not set</span>
                          )}
                        </td>
                      )}
                      <td className="price-cell">
        ...
        >
          <div className="form-grid">
            <div className="form-group full">
              ...
              <input
                ...
                onChange={(e) =>
                  setEntryForm((p) => ({ ...p, stockDate: e.target.value }))
                }
              />
            </div>
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setEntryForm((p) => ({ ...p, count: e.target.value }))
                }
              />
            </div>
            <div className="form-group">
            ...
            {isAdmin && (
              <div className="form-group">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEntryForm((p) => ({ ...p, price: e.target.value }))
                  }
                />
              </div>
            )}
            <div className="form-group">
              ...
              <input
                ...
                onChange={(e) =>
                  setEntryForm((p) => ({ ...p, retailPrice: e.target.value }))
                }
              />
            </div>
            <div className="form-group full">
          ...
          {Number(entryForm.remainingCount) === 0 && (
            <div className="info-hint" style={{ marginTop: 12 }}>
              ...
              Remaining count is <strong style={{ color: "#ef4444" }}>0</strong>{" "}
              — this batch will be marked as fully sold out.
            </div>
          )}
          {formError && (
            <p className="ferr" style={{ marginTop: 14 }}>
        ...
        >
          <p style={{ fontSize: 14, color: "#94a3b8", lineHeight: 1.7 }}>
            ...
            <strong style={{ color: "#f1f5f9" }}>
              {selectedEntry.count} units
            </strong>{" "}
            added on{" "}
            <strong style={{ color: "#f1f5f9" }}>
              {fmtDate(selectedEntry.stockDate || selectedEntry.createdAt)}
            </strong>
            ?
          </p>
          <div className="info-hint" style={{ marginTop: 14 }}>
            ...
            <strong style={{ color: "#c7d2fe" }}>
              {selectedEntry.remainingCount} unsold units
            </strong>{" "}
      ...
      {modal === "view" &&
        ...
        (() => {
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
          const pImages = toProductImages((selected as any).images);
          return (
            <Modal
              title={
                <div className="modal-title-wrap">
                  ...
                  {selected.flavour && selected.flavour !== "none" && (
                    <span className="modal-flavour-badge">
                      {selected.flavour}
                    </span>
                  )}
                </div>
            ...
            >
              {/* Image gallery — shown only if product has images */}
              {pImages.length > 0 && (
                <div style={{ marginBottom: 20 }}>
                  <ProductImageStrip images={pImages} variant="strip" />
                </div>
              )}
              ...
              <div className="detail-grid">
                ...
                <div className="detail-item">
                  ...
                  <span
                    ...
                    style={{
                      color: stockColor(selected.currentStock ?? 0).color,
                    }}
                  >
                    {selected.currentStock ?? 0}
                ...
                <div className="detail-item">
                  ...
                  <span className="detail-val">
                    {selected.stockSummary?.totalBatches ?? 0}
                  </span>
                </div>
                ...
                {isAdmin && selected.priceSummary && (
                  <>
                    <div className="detail-item">
                      ...
                      <span className="detail-val mono">
                        {fmt(selected.priceSummary.latestPrice)}
                      </span>
                    </div>
                    {selected.priceSummary.priceRange.min !==
                      selected.priceSummary.priceRange.max && (
                      <div className="detail-item full">
                        <span className="detail-label">
                          Cost Price Range (active batches)
                        </span>
                        <span className="detail-val mono">
                          {fmt(selected.priceSummary.priceRange.min)} –{" "}
                          {fmt(selected.priceSummary.priceRange.max)}
                        </span>
                      </div>
                    )}
                  </>
                )}
                {selected.priceSummary != null && (
                  <hr className="detail-divider" />
                )}
      ...
      {modal === "delete" && selected && (
        ...
        >
          <p style={{ fontSize: 14, color: "#94a3b8", lineHeight: 1.7 }}>
            ...
            <strong style={{ color: "#f1f5f9" }}>{selected.name}</strong>? This
            will mark it as deleted and cannot be undone.
          </p>
        </Modal>
      )}
